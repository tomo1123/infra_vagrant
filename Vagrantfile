...
sudo apt-get update
sudo apt-get -y upgrade
sudo apt-get -y install build-essential libssl-dev libffi-dev libxml2-dev libxslt1-dev cargo sshpass
sudo apt-get -y install python-dev python3-dev python3-pip
sudo pip3 install --upgrade pip
sudo apt-get install --reinstall ca-certificates
cd /opt && sudo wget https://www.python.org/ftp/python/3.6.9/Python-3.6.9.tgz --no-check-certificate && sudo tar zxvf Python-3.6.9.tgz && sudo chown -R ${USER} Python-3.6.9 && cd Python-3.6.9 && ./configure && make && sudo make install && sudo ln -fs /opt/Python-3.6.9/python /usr/bin/python && cd
